...
<body>
    ...
    <div id="mainDiv" class="container" style="margin-top: 35px; ">
        ...
        <div class="well">
            ...
            <span class="label label-danger" style="margin-left: 4%;">还有${remainValidDays}天过期</span>
            <span class="label label-success" style="margin-left: 4%;">课件资料：${card.courseware}</span>  
        </div>
        
        <!-- 此学习卡包含的课程章节列表 -->
        <div class="panel-group" id="accordion">
           <c:forEach items="${chapterSegmentListMap}" var="entry">
                <div class="panel panel-default">
                    ...
                    <c:if test="${latestSegmentId==segment.id}">in</c:if>
                     </c:forEach>
                    " aria-expanded="true"> 
                        <div class="panel-body">
                            ...
                            <c:forEach items="${entry.value}" var="segment">
                                    <a class="list-group-item" href="<%=ctxPath%>/segment/detail.do?id=${segment.id}" target="_blank">
                                        ...
                                        <c:if test="${latestSegmentId==segment.id}">
