...
<div class="pd-20">
    ...
    <div class="mt-20">
        <table class="table table-border table-bordered table-bg table-hover">
            ...
            <tbody>
            <c:forEach items="${pageInfoList.list}" var="user">
                    <tr>
                        ...
                        <td>
                            ...
                            <button class="btn size-MINI radius" onclick="ajaxSubmit('<%=ctxPath%>/user/setTeacher.do','id=${user.id}&isTeacher=${!user.isTeacher}')">${user.isTeacher?'取消':''}设为老师</button>
                        </td>
                    </tr>
            </c:forEach>        
            </tbody>
        </table>
        <c:if test="${pageInfoList.pages>1 }">
            <div id="pagination" style="margin: 20px;"></div>
            <script type="text/javascript" src="<%=ctxPath%>/lib/laypage/1.2/laypage.js"></script>
            <script type="text/javascript">
            laypage({
                cont:'pagination',
                pages:${pageInfoList.pages},
                curr:${pageInfoList.pageNum},
